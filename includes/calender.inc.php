...
//load function bades on the Ajax request
...
//generate event calender in html
            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        global $conn;
                        global $userid;
                        $query = pg_query_params($conn, "SELECT COUNT(title) AS amount FROM calender WHERE date = $1 AND status = 1 AND userid = $2",array($currentDate, $userid)); 
                        $result = pg_fetch_array($query);
                        $eventNum = $result['amount'];
...
function getEvents($date = ''){ 
    ...
    global $conn;
    global $userid;
    $result = pg_query_params($conn, "SELECT id, title FROM calender WHERE date = $1 AND status = 1 AND userid = $2",array($date, $userid)); 
    $qry = pg_query_params($conn, "SELECT COUNT(title) AS amount FROM calender WHERE date = $1 AND status = 1 AND userid = $2",array($date, $userid));
    ...
    if($rowcount['amount'] > 0){ 
        ...
        $i=0; 
        while($row = pg_fetch_array($result,NULL, PGSQL_ASSOC)){ $i++; 
            $eventListHTML .= "<a href='/calender.php?id=".$row['id']."' ><li id='editrow' class='sidebar__list-item listitem'><span class='list-item__time'>".$i.".</span>".$row['title']."</li></a>"; 
        } 
        $eventListHTML .= '</ul>'; 
    } 
    echo $eventListHTML;
